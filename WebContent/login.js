...
function setAnswerBackground(correctA) {
	...
	//sofern die gewählte Antwort auch die richtige Antwort war -> richtige Antwort GRÜN hinterlegen
	if (sendAnswer === correctA) {
		document.getElementById("answer" + correctA).style.background = "#BEF781";
		
	//wenn gar keine Antwort gewählt wurde -> richtige Antwort GRÜN hinterlegen
	} else if (sendAnswer === 4) {
		document.getElementById("answer" + correctA).style.background = "#BEF781";
		
	//webb die gegebene Antwort falsch war -> richtige Antwort GRÜN, falsch Antwort (sendAnswer) ROT
	} else {
...
//die "main"-Section wird von allen ChildNodes befreit
function cleanMain() {
...
function createQuestion(questionFromServer) {
	...
	//auffüllen der Tabelle mit den Antworten
	...
	//timeout wird ausgelesen
	timer = questionFromServer[5] / 1000;
	question = questionFromServer[0];
	
	//schreiben des Fragetextes in das Frage-Feld
...
//erstellen der Tabelle für die Fragen
function tableCreate() {
	...
	//erstellen der verschiedenen Zeilen und Spalten
	var aC = 0;
	for (var i = 0; i < 3; i++) {
	...
	//anhängen der Tabelle an die "main"-Section
	questionSection.appendChild(tbl);
}

//der Login-Button wird immer entfernt, sobald sich ein Spieler angemeldet hat, damit keine
//weitere Loginfunktion mehr möglich ist
function removeLoginButton() {
	...
	//hinzufügen eines Textes, der bis zum Spielstart zum Warten auffordert
...
//der Spielleiter bekommt einen anderen Text vor dem Spielstart, der ihn auffordert einen Katalog zu wählen 
//und das er die Möglichkeit hat bei Anmeldung eines zweiten Spielers das Spiel zu starten 
...
//diese Funktion realisiert den Timer, der auf eine Animation zurückgreift
function initTimer() {
	animation = window.setInterval("countTime();", 1000);
}

//aktualisiert den Timer und schreibt die verbleibende Zeit unter den Fragetext
function countTime() {

	var questionText = document.getElementById("questionText");
	timer = timer - 1;
	
	//sobald der Timer ohne Antwortwahl ausläuft, wird er gestoppt, die Antwort "4" an den Server
	//gesendet und auf eine Anwort (Lösung der Frage gewartet)
	if (timer == 0) {
	...
	//aktualisieren der Timeranzeige	
	} else {
		questionText.innerHTML = question + "<br /><br /><br />"
				+ "Seconds left:  " + timer;
	}
...
//sendet ein RESPONSE-Paket an den Server mit der gewählten Antwort 
function answerClicked(event) {
	...
	if (!waitingForResponse) {
		for (var i = 0; i < 4; i++) {
			...
			//rausfinden der angeklickten Antwort
			if (answerClicked === document.getElementById("answer" + i)) {
				...
				//durch das Setzen dieser Variablen ist kein weiteres Klicken auf die Antworten
				//mehr möglich
				waitingForResponse = true;
			}
		}
...
//erstellen der Highscoreliste mithilfe einer Tabelle
...
//nachdem ein Spiele alle Fragen beendet hat, wartet der Server bis alle Spieler fertig sind und er
//den Rang der Spieler versenden kann. Bis dahin wird der Spieler mit dem Text aufgefordert zu warten
function setMainTextWait() {
	
	//löschen aller Elemente in der "main"-Section und hinzufügen eines Text-divs
	cleanMain();
...
//nachdem alle Spieler fertig sind, bekommen sie ihren Rang angezeigt
